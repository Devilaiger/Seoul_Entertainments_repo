...
import com.seoulentertainment.app.newAudioFile
...
class BilibiliProvider : MainAPI() {
    ...
    companion object {Kkokoreankorean
    ...
    private suspend fun checkContentAccess(epId: String?, aid: String?): ContentAccessError {
        try {
            ...
            return when {
                // Code 10015001 = geo-restricted ("??????")
                json.code == 10015001 || json.message?.contains("??") == true || json.message?.contains("region") == true -> ContentAccessError.GEO_LOCKED
        ...
        try {
            ...
            if (firstEpId != null) {
                when (checkContentAccess(firstEpId, null)) {
                    ContentAccessError.GEO_LOCKED -> throw ErrorLoadingException("?? GEO-LOCKED: This content is not available in your region. Use a VPN to access content from Southeast Asia (Thailand, Indonesia, Vietnam, etc.)")
                    ContentAccessError.PREMIUM_REQUIRED -> throw ErrorLoadingException("?? PREMIUM CONTENT: This content requires a Bilibili TV subscription to watch.")
    ...
    private suspend fun tryPlayurlApi(epId: String?, aid: String?, callback: (ExtractorLink) -> Unit): Boolean {
        try {
            ...
            if (json.code != 0) {
                ...
                // Check for geo-restriction error (code 10015001 = "??????" / Copyright region restricted)
                if (json.code == 10015001 || json.message?.contains("??") == true || json.message?.contains("region") == true) {
                    // Show geo-locked message to user
                    callback.invoke(
                        newExtractorLink(
                            source = name,
                            name = "?? GEO-LOCKED - Content not available in your region",
                ...
                if (json.code == 10004004) {
                    callback.invoke(
                        newExtractorLink(
                            source = name,
                            name = "?? PREMIUM CONTENT - Requires Bilibili TV subscription",
